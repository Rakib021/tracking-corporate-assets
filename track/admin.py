...
class AssetsModelAdmin(admin.ModelAdmin):
    list_display = ['id', 'asset_name', 'asset_manufacturer',
                    'asset_manufacturer', 'asset_condition', 'asset_issued']


@admin.register(AssetsLog)
class AsssetsLogAdmin(admin.ModelAdmin):
    list_display = ['id', 'asset', 'employee', 'checkout_date',
                    'return_date', 'checkout_condition', 'return_condition']
